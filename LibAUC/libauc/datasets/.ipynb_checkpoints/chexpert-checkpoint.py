...
class CheXpert(Dataset):
    ...
    def __init__(self, 
        ...
        self_df = self.df['Path'].tolist()
        self_images_list = []
        for path in self_df:
            path_piece = path.split('/')
            self_images_list.append(image_root_path + os.path.join(path_piece[0],path_piece[1],path_piece[2],path_piece[3]))
        self._images_list = self_images_list
...
if __name__ == '__main__':
    ...
    traindSet = CheXpert(root+'train.csv', image_root_path=root, use_upsampling=True, use_frontal=True, image_size=320, mode='train', class_index=0)
    testSet =  CheXpert(root+'valid.csv',  image_root_path=root, use_upsampling=False, use_frontal=True, image_size=320, mode='valid', class_index=0)
